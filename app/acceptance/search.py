

def generate_search_tests():
    return {
        'actions': {
            'naics_bmo': {
                'params': {'naics': '238220'},
                'action': ('#naics-code', 'select[238220]'),
                'naics': ('238220', 78, True),
                'vehicle': ('all', 4, True),
                'pool': ('all', 8, 7, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (25, 'results/csv/?naics=238220'),
                'table': (25, 'h_naics_results', 'desc')
            },
            'naics_bmo_sb': {
                'params': {'naics': '561730'},
                'action': ('#naics-code', 'select[561730]'),
                'naics': ('561730', 78, True),
                'vehicle': ('all', 3, True),
                'pool': ('all', 5, 4, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (15, 'results/csv/?naics=561730'),
                'table': (15, 'h_naics_results', 'desc')
            },
            'naics_hcats': {
                'params': {'naics': '611710'},
                'action': ('#naics-code', 'select[611710]'),
                'naics': ('611710', 78, True),
                'vehicle': ('all', 4, True),
                'pool': ('all', 4, 3, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (28, 'results/csv/?naics=611710'),
                'table': (28, 'h_naics_results', 'desc')
            },
            'naics_hcats_sb': {
                'params': {'naics': '541618'},
                'action': ('#naics-code', 'select[541618]'),
                'naics': ('541618', 78, True),
                'vehicle': ('all', 5, True),
                'pool': ('all', 5, 4, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (37, 'results/csv/?naics=541618'),
                'table': (37, 'h_naics_results', 'desc')
            },
            'naics_oasis': {
                'params': {'naics': '541712'},
                'action': ('#naics-code', 'select[541712]'),
                'naics': ('541712', 78, True),
                'vehicle': ('all', 4, True),
                'pool': ('all', 10, 9, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (50, 'results/csv/?naics=541712'),
                'table': (50, 'h_naics_results', 'desc')
            },
            'naics_oasis_sb': {
                'params': {'naics': '541810'},
                'action': ('#naics-code', 'select[541810]'),
                'naics': ('541810', 78, True),
                'vehicle': ('all', 4, True),
                'pool': ('all', 4, 3, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (30, 'results/csv/?naics=541810'),
                'table': (30, 'h_naics_results', 'desc')
            },
            'naics_pss': {
                'params': {'naics': '236220'},
                'action': ('#naics-code', 'select[236220]'),
                'naics': ('236220', 78, True),
                'vehicle': ('all', 4, True),
                'pool': ('all', 4, 3, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (24, 'results/csv/?naics=236220'),
                'table': (24, 'h_naics_results', 'desc')
            },
            
            'vehicle_bmo': {
                'params': {'vehicle': 'BMO'},
                'action': ('#vehicle-id', 'select[BMO]'),
                'naics': ('all', 15, True),
                'vehicle': ('BMO', 8, True),
                'pool': ('all', 18, 17, True),
                'zone': ('all', 7, True, True),
                'filters': (None, 0, True),
                'results': (12, 'results/csv/?vehicle=BMO'),
                'table': (12, 'h_naics_results', 'desc')
            },
            'vehicle_bmo_sb': {
                'params': {'vehicle': 'BMO_SB'},
                'action': ('#vehicle-id', 'select[BMO_SB]'),
                'naics': ('all', 15, True),
                'vehicle': ('BMO_SB', 8, True),
                'pool': ('all', 18, 17, True),
                'zone': ('all', 7, True, True),
                'filters': (None, 0, True),
                'results': (13, 'results/csv/?vehicle=BMO_SB'),
                'table': (13, 'h_naics_results', 'desc')
            },
            'vehicle_hcats': {
                'params': {'vehicle': 'HCATS'},
                'action': ('#vehicle-id', 'select[HCATS]'),
                'naics': ('all', 9, True),
                'vehicle': ('HCATS', 8, True),
                'pool': ('all', 3, 2, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (12, 'results/csv/?vehicle=HCATS'),
                'table': (12, 'h_naics_results', 'desc')
            },
            'vehicle_hcats_sb': {
                'params': {'vehicle': 'HCATS_SB'},
                'action': ('#vehicle-id', 'select[HCATS_SB]'),
                'naics': ('all', 9, True),
                'vehicle': ('HCATS_SB', 8, True),
                'pool': ('all', 3, 2, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (17, 'results/csv/?vehicle=HCATS_SB'),
                'table': (17, 'h_naics_results', 'desc')
            },
            'vehicle_oasis': {
                'params': {'vehicle': 'OASIS'},
                'action': ('#vehicle-id', 'select[OASIS]'),
                'naics': ('all', 29, True),
                'vehicle': ('OASIS', 8, True),
                'pool': ('all', 8, 7, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (29, 'results/csv/?vehicle=OASIS'),
                'table': (29, 'h_naics_results', 'desc')
            },
            'vehicle_oasis_sb': {
                'params': {'vehicle': 'OASIS_SB'},
                'action': ('#vehicle-id', 'select[OASIS_SB]'),
                'naics': ('all', 29, True),
                'vehicle': ('OASIS_SB', 8, True),
                'pool': ('all', 8, 7, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (46, 'results/csv/?vehicle=OASIS_SB'),
                'table': (46, 'h_naics_results', 'desc')
            },
            'vehicle_pss': {
                'params': {'vehicle': 'PSS'},
                'action': ('#vehicle-id', 'select[PSS]'),
                'naics': ('all', 58, True),
                'vehicle': ('PSS', 8, True),
                'pool': ('all', 8, 7, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (66, 'results/csv/?vehicle=PSS'),
                'table': (50, 'h_naics_results', 'desc', ('Prev', '1'), '2')
            },
            
            'pool_bmo_1': {
                'params': {'pool': 'BMO_1'},
                'action': ('#pool-id', 'select[BMO_1]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_1', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (7, 'results/csv/?pool=BMO_1'),
                'table': (7, 'h_naics_results', 'desc')
            },
            'pool_bmo_2': {
                'params': {'pool': 'BMO_2'},
                'action': ('#pool-id', 'select[BMO_2]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_2', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (7, 'results/csv/?pool=BMO_2'),
                'table': (7, 'h_naics_results', 'desc')
            },
            'pool_bmo_3': {
                'params': {'pool': 'BMO_3'},
                'action': ('#pool-id', 'select[BMO_3]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_3', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (7, 'results/csv/?pool=BMO_3'),
                'table': (7, 'h_naics_results', 'desc')
            },
            'pool_bmo_4': {
                'params': {'pool': 'BMO_4'},
                'action': ('#pool-id', 'select[BMO_4]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_4', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (7, 'results/csv/?pool=BMO_4'),
                'table': (7, 'h_naics_results', 'desc')
            },
            'pool_bmo_5': {
                'params': {'pool': 'BMO_5'},
                'action': ('#pool-id', 'select[BMO_5]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_5', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (7, 'results/csv/?pool=BMO_5'),
                'table': (7, 'h_naics_results', 'desc')
            },
            'pool_bmo_6': {
                'params': {'pool': 'BMO_6'},
                'action': ('#pool-id', 'select[BMO_6]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_6', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (9, 'results/csv/?pool=BMO_6'),
                'table': (9, 'h_naics_results', 'desc')
            },
            'pool_bmo_7': {
                'params': {'pool': 'BMO_7'},
                'action': ('#pool-id', 'select[BMO_7]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_7', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (7, 'results/csv/?pool=BMO_7'),
                'table': (7, 'h_naics_results', 'desc')
            },
            'pool_bmo_8': {
                'params': {'pool': 'BMO_8'},
                'action': ('#pool-id', 'select[BMO_8]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_8', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (7, 'results/csv/?pool=BMO_8'),
                'table': (7, 'h_naics_results', 'desc')
            },
            'pool_bmo_9': {
                'params': {'pool': 'BMO_9'},
                'action': ('#pool-id', 'select[BMO_9]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_9', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (7, 'results/csv/?pool=BMO_9'),
                'table': (7, 'h_naics_results', 'desc')
            },
            'pool_bmo_10': {
                'params': {'pool': 'BMO_10'},
                'action': ('#pool-id', 'select[BMO_10]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_10', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (7, 'results/csv/?pool=BMO_10'),
                'table': (7, 'h_naics_results', 'desc')
            },
            'pool_bmo_11': {
                'params': {'pool': 'BMO_11'},
                'action': ('#pool-id', 'select[BMO_11]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_11', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (7, 'results/csv/?pool=BMO_11'),
                'table': (7, 'h_naics_results', 'desc')
            },
            'pool_bmo_12': {
                'params': {'pool': 'BMO_12'},
                'action': ('#pool-id', 'select[BMO_12]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_12', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (7, 'results/csv/?pool=BMO_12'),
                'table': (7, 'h_naics_results', 'desc')
            },
            'pool_bmo_13': {
                'params': {'pool': 'BMO_13'},
                'action': ('#pool-id', 'select[BMO_13]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_13', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (7, 'results/csv/?pool=BMO_13'),
                'table': (7, 'h_naics_results', 'desc')
            },
            'pool_bmo_14': {
                'params': {'pool': 'BMO_14'},
                'action': ('#pool-id', 'select[BMO_14]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_14', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (8, 'results/csv/?pool=BMO_14'),
                'table': (8, 'h_naics_results', 'desc')
            },
            'pool_bmo_15': {
                'params': {'pool': 'BMO_15'},
                'action': ('#pool-id', 'select[BMO_15]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_15', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (9, 'results/csv/?pool=BMO_15'),
                'table': (9, 'h_naics_results', 'desc')
            },
            'pool_bmo_16': {
                'params': {'pool': 'BMO_16'},
                'action': ('#pool-id', 'select[BMO_16]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_16', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (9, 'results/csv/?pool=BMO_16'),
                'table': (9, 'h_naics_results', 'desc')
            },
            'pool_bmo_17': {
                'params': {'pool': 'BMO_17'},
                'action': ('#pool-id', 'select[BMO_17]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_17', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (3, 'results/csv/?pool=BMO_17'),
                'table': (3, 'h_naics_results', 'desc')
            },
                    
            'pool_bmo_sb_1': {
                'params': {'pool': 'BMO_SB_1'},
                'action': ('#pool-id', 'select[BMO_SB_1]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_SB_1', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (6, 'results/csv/?pool=BMO_SB_1'),
                'table': (6, 'h_naics_results', 'desc')
            },
            'pool_bmo_sb_2': {
                'params': {'pool': 'BMO_SB_2'},
                'action': ('#pool-id', 'select[BMO_SB_2]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_SB_2', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (6, 'results/csv/?pool=BMO_SB_2'),
                'table': (6, 'h_naics_results', 'desc')
            },
            'pool_bmo_sb_3': {
                'params': {'pool': 'BMO_SB_3'},
                'action': ('#pool-id', 'select[BMO_SB_3]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_SB_3', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (6, 'results/csv/?pool=BMO_SB_3'),
                'table': (6, 'h_naics_results', 'desc')
            },
            'pool_bmo_sb_4': {
                'params': {'pool': 'BMO_SB_4'},
                'action': ('#pool-id', 'select[BMO_SB_4]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_SB_4', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (6, 'results/csv/?pool=BMO_SB_4'),
                'table': (6, 'h_naics_results', 'desc')
            },
            'pool_bmo_sb_5': {
                'params': {'pool': 'BMO_SB_5'},
                'action': ('#pool-id', 'select[BMO_SB_5]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_SB_5', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (6, 'results/csv/?pool=BMO_SB_5'),
                'table': (6, 'h_naics_results', 'desc')
            },
            'pool_bmo_sb_6': {
                'params': {'pool': 'BMO_SB_6'},
                'action': ('#pool-id', 'select[BMO_SB_6]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_SB_6', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (6, 'results/csv/?pool=BMO_SB_6'),
                'table': (6, 'h_naics_results', 'desc')
            },
            'pool_bmo_sb_7': {
                'params': {'pool': 'BMO_SB_7'},
                'action': ('#pool-id', 'select[BMO_SB_7]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_SB_7', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (6, 'results/csv/?pool=BMO_SB_7'),
                'table': (6, 'h_naics_results', 'desc')
            },
            'pool_bmo_sb_8': {
                'params': {'pool': 'BMO_SB_8'},
                'action': ('#pool-id', 'select[BMO_SB_8]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_SB_8', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (7, 'results/csv/?pool=BMO_SB_8'),
                'table': (7, 'h_naics_results', 'desc')
            },
            'pool_bmo_sb_9': {
                'params': {'pool': 'BMO_SB_9'},
                'action': ('#pool-id', 'select[BMO_SB_9]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_SB_9', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (7, 'results/csv/?pool=BMO_SB_9'),
                'table': (7, 'h_naics_results', 'desc')
            },
            'pool_bmo_sb_10': {
                'params': {'pool': 'BMO_SB_10'},
                'action': ('#pool-id', 'select[BMO_SB_10]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_SB_10', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (7, 'results/csv/?pool=BMO_SB_10'),
                'table': (7, 'h_naics_results', 'desc')
            },
            'pool_bmo_sb_11': {
                'params': {'pool': 'BMO_SB_11'},
                'action': ('#pool-id', 'select[BMO_SB_11]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_SB_11', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (7, 'results/csv/?pool=BMO_SB_11'),
                'table': (7, 'h_naics_results', 'desc')
            },
            'pool_bmo_sb_12': {
                'params': {'pool': 'BMO_SB_12'},
                'action': ('#pool-id', 'select[BMO_SB_12]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_SB_12', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (9, 'results/csv/?pool=BMO_SB_12'),
                'table': (9, 'h_naics_results', 'desc')
            },
            'pool_bmo_sb_13': {
                'params': {'pool': 'BMO_SB_13'},
                'action': ('#pool-id', 'select[BMO_SB_13]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_SB_13', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (9, 'results/csv/?pool=BMO_SB_13'),
                'table': (9, 'h_naics_results', 'desc')
            },
            'pool_bmo_sb_14': {
                'params': {'pool': 'BMO_SB_14'},
                'action': ('#pool-id', 'select[BMO_SB_14]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_SB_14', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (9, 'results/csv/?pool=BMO_SB_14'),
                'table': (9, 'h_naics_results', 'desc')
            },
            'pool_bmo_sb_15': {
                'params': {'pool': 'BMO_SB_15'},
                'action': ('#pool-id', 'select[BMO_SB_15]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_SB_15', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (9, 'results/csv/?pool=BMO_SB_15'),
                'table': (9, 'h_naics_results', 'desc')
            },
            'pool_bmo_sb_16': {
                'params': {'pool': 'BMO_SB_16'},
                'action': ('#pool-id', 'select[BMO_SB_16]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_SB_16', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (9, 'results/csv/?pool=BMO_SB_16'),
                'table': (9, 'h_naics_results', 'desc')
            },
            'pool_bmo_sb_17': {
                'params': {'pool': 'BMO_SB_17'},
                'action': ('#pool-id', 'select[BMO_SB_17]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('BMO_SB_17', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (1, 'results/csv/?pool=BMO_SB_17'),
                'table': (1, 'h_naics_results', 'desc')
            },
            'pool_hcats_1': {
                'params': {'pool': 'HCATS_1'},
                'action': ('#pool-id', 'select[HCATS_1]'),
                'naics': ('all', 4, True),
                'vehicle': ('all', 8, True),
                'pool': ('HCATS_1', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (10, 'results/csv/?pool=HCATS_1'),
                'table': (10, 'h_naics_results', 'desc')
            },
            'pool_hcats_2': {
                'params': {'pool': 'HCATS_2'},
                'action': ('#pool-id', 'select[HCATS_2]'),
                'naics': ('all', 6, True),
                'vehicle': ('all', 8, True),
                'pool': ('HCATS_2', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (10, 'results/csv/?pool=HCATS_2'),
                'table': (10, 'h_naics_results', 'desc')
            },
            'pool_hcats_sb_1': {
                'params': {'pool': 'HCATS_SB_1'},
                'action': ('#pool-id', 'select[HCATS_SB_1]'),
                'naics': ('all', 4, True),
                'vehicle': ('all', 8, True),
                'pool': ('HCATS_SB_1', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (10, 'results/csv/?pool=HCATS_SB_1'),
                'table': (10, 'h_naics_results', 'desc')
            },
            'pool_hcats_sb_2': {
                'params': {'pool': 'HCATS_SB_2'},
                'action': ('#pool-id', 'select[HCATS_SB_2]'),
                'naics': ('all', 6, True),
                'vehicle': ('all', 8, True),
                'pool': ('HCATS_SB_2', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (10, 'results/csv/?pool=HCATS_SB_2'),
                'table': (10, 'h_naics_results', 'desc')
            },
            'pool_oasis_1': {
                'params': {'pool': 'OASIS_1'},
                'action': ('#pool-id', 'select[OASIS_1]'),
                'naics': ('all', 22, True),
                'vehicle': ('all', 8, True),
                'pool': ('OASIS_1', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (10, 'results/csv/?pool=OASIS_1'),
                'table': (10, 'h_naics_results', 'desc')
            },
            'pool_oasis_2': {
                'params': {'pool': 'OASIS_2'},
                'action': ('#pool-id', 'select[OASIS_2]'),
                'naics': ('all', 6, True),
                'vehicle': ('all', 8, True),
                'pool': ('OASIS_2', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (10, 'results/csv/?pool=OASIS_2'),
                'table': (10, 'h_naics_results', 'desc')
            },
            'pool_oasis_3': {
                'params': {'pool': 'OASIS_3'},
                'action': ('#pool-id', 'select[OASIS_3]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('OASIS_3', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (10, 'results/csv/?pool=OASIS_3'),
                'table': (10, 'h_naics_results', 'desc')
            },
            'pool_oasis_4': {
                'params': {'pool': 'OASIS_4'},
                'action': ('#pool-id', 'select[OASIS_4]'),
                'naics': ('all', 3, True),
                'vehicle': ('all', 8, True),
                'pool': ('OASIS_4', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (10, 'results/csv/?pool=OASIS_4'),
                'table': (10, 'h_naics_results', 'desc')
            },
            'pool_oasis_5a': {
                'params': {'pool': 'OASIS_5A'},
                'action': ('#pool-id', 'select[OASIS_5A]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('OASIS_5A', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (10, 'results/csv/?pool=OASIS_5A'),
                'table': (10, 'h_naics_results', 'desc')
            },
            'pool_oasis_5b': {
                'params': {'pool': 'OASIS_5B'},
                'action': ('#pool-id', 'select[OASIS_5B]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('OASIS_5B', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (10, 'results/csv/?pool=OASIS_5B'),
                'table': (10, 'h_naics_results', 'desc')
            },
            'pool_oasis_6': {
                'params': {'pool': 'OASIS_6'},
                'action': ('#pool-id', 'select[OASIS_6]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('OASIS_6', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (10, 'results/csv/?pool=OASIS_6'),
                'table': (10, 'h_naics_results', 'desc')
            },
            'pool_oasis_sb_1': {
                'params': {'pool': 'OASIS_SB_1'},
                'action': ('#pool-id', 'select[OASIS_SB_1]'),
                'naics': ('all', 22, True),
                'vehicle': ('all', 8, True),
                'pool': ('OASIS_SB_1', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (10, 'results/csv/?pool=OASIS_SB_1'),
                'table': (10, 'h_naics_results', 'desc')
            },
            'pool_oasis_sb_2': {
                'params': {'pool': 'OASIS_SB_2'},
                'action': ('#pool-id', 'select[OASIS_SB_2]'),
                'naics': ('all', 6, True),
                'vehicle': ('all', 8, True),
                'pool': ('OASIS_SB_2', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (10, 'results/csv/?pool=OASIS_SB_2'),
                'table': (10, 'h_naics_results', 'desc')
            },
            'pool_oasis_sb_3': {
                'params': {'pool': 'OASIS_SB_3'},
                'action': ('#pool-id', 'select[OASIS_SB_3]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('OASIS_SB_3', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (10, 'results/csv/?pool=OASIS_SB_3'),
                'table': (10, 'h_naics_results', 'desc')
            },
            'pool_oasis_sb_4': {
                'params': {'pool': 'OASIS_SB_4'},
                'action': ('#pool-id', 'select[OASIS_SB_4]'),
                'naics': ('all', 3, True),
                'vehicle': ('all', 8, True),
                'pool': ('OASIS_SB_4', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (10, 'results/csv/?pool=OASIS_SB_4'),
                'table': (10, 'h_naics_results', 'desc')
            },
            'pool_oasis_sb_5a': {
                'params': {'pool': 'OASIS_SB_5A'},
                'action': ('#pool-id', 'select[OASIS_SB_5A]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('OASIS_SB_5A', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (10, 'results/csv/?pool=OASIS_SB_5A'),
                'table': (10, 'h_naics_results', 'desc')
            },
            'pool_oasis_sb_5b': {
                'params': {'pool': 'OASIS_SB_5B'},
                'action': ('#pool-id', 'select[OASIS_SB_5B]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('OASIS_SB_5B', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (10, 'results/csv/?pool=OASIS_SB_5B'),
                'table': (10, 'h_naics_results', 'desc')
            },
            'pool_oasis_sb_6': {
                'params': {'pool': 'OASIS_SB_6'},
                'action': ('#pool-id', 'select[OASIS_SB_6]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('OASIS_SB_6', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (10, 'results/csv/?pool=OASIS_SB_6'),
                'table': (10, 'h_naics_results', 'desc')
            },
            'pool_pss_382': {
                'params': {'pool': 'PSS_382'},
                'action': ('#pool-id', 'select[PSS_382]'),
                'naics': ('all', 2, True),
                'vehicle': ('all', 8, True),
                'pool': ('PSS_382', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (10, 'results/csv/?pool=PSS_382'),
                'table': (10, 'h_naics_results', 'desc')
            },
            'pool_pss_520': {
                'params': {'pool': 'PSS_520'},
                'action': ('#pool-id', 'select[PSS_520]'),
                'naics': ('all', 13, True),
                'vehicle': ('all', 8, True),
                'pool': ('PSS_520', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (10, 'results/csv/?pool=PSS_520'),
                'table': (10, 'h_naics_results', 'desc')
            },
            'pool_pss_541': {
                'params': {'pool': 'PSS_541'},
                'action': ('#pool-id', 'select[PSS_541]'),
                'naics': ('all', 11, True),
                'vehicle': ('all', 8, True),
                'pool': ('PSS_541', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (10, 'results/csv/?pool=PSS_541'),
                'table': (10, 'h_naics_results', 'desc')
            },
            'pool_pss_871': {
                'params': {'pool': 'PSS_871'},
                'action': ('#pool-id', 'select[PSS_871]'),
                'naics': ('all', 5, True),
                'vehicle': ('all', 8, True),
                'pool': ('PSS_871', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (10, 'results/csv/?pool=PSS_871'),
                'table': (10, 'h_naics_results', 'desc')
            },
            'pool_pss_874': {
                'params': {'pool': 'PSS_874'},
                'action': ('#pool-id', 'select[PSS_874]'),
                'naics': ('all', 16, True),
                'vehicle': ('all', 8, True),
                'pool': ('PSS_874', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (10, 'results/csv/?pool=PSS_874'),
                'table': (10, 'h_naics_results', 'desc')
            },
            'pool_pss_874500': {
                'params': {'pool': 'PSS_874500'},
                'action': ('#pool-id', 'select[PSS_874500]'),
                'naics': ('all', 11, True),
                'vehicle': ('all', 8, True),
                'pool': ('PSS_874500', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (10, 'results/csv/?pool=PSS_874500'),
                'table': (10, 'h_naics_results', 'desc')
            },
            'pool_pss_899': {
                'params': {'pool': 'PSS_899'},
                'action': ('#pool-id', 'select[PSS_899]'),
                'naics': ('all', 7, True),
                'vehicle': ('all', 8, True),
                'pool': ('PSS_899', 60, 1, True),
                'zone': ('all', 7, False, False),
                'filters': (None, 0, True),
                'results': (10, 'results/csv/?pool=PSS_899'),
                'table': (10, 'h_naics_results', 'desc')
            },
            
            'bmo_zone_1': {
                'params': {'vehicle': 'BMO', 'zone': '1'},
                'action': (('#vehicle-id', 'select[BMO]'), ('#zone-id', 'select[1]')),
                'naics': ('all', 15, True),
                'vehicle': ('BMO', 8, True),
                'pool': ('all', 18, 17, True),
                'zone': ('1', 7, True, True),
                'filters': (None, 0, True),
                'results': (7, 'results/csv/?vehicle=BMO&zone=1'),
                'table': (7, 'h_naics_results', 'desc')
            },
            'bmo_sb_zone_2': {
                'params': {'vehicle': 'BMO_SB', 'zone': '2'},
                'action': (('#vehicle-id', 'select[BMO_SB]'), ('#zone-id', 'select[2]')),
                'naics': ('all', 15, True),
                'vehicle': ('BMO_SB', 8, True),
                'pool': ('all', 18, 17, True),
                'zone': ('2', 7, True, True),
                'filters': (None, 0, True),
                'results': (4, 'results/csv/?vehicle=BMO_SB&zone=2'),
                'table': (4, 'h_naics_results', 'desc')
            },
            'bmo_zone_3': {
                'params': {'vehicle': 'BMO', 'zone': '3'},
                'action': (('#vehicle-id', 'select[BMO]'), ('#zone-id', 'select[3]')),
                'naics': ('all', 15, True),
                'vehicle': ('BMO', 8, True),
                'pool': ('all', 18, 17, True),
                'zone': ('3', 7, True, True),
                'filters': (None, 0, True),
                'results': (7, 'results/csv/?vehicle=BMO&zone=3'),
                'table': (7, 'h_naics_results', 'desc')
            },
            'bmo_sb_zone_4': {
                'params': {'vehicle': 'BMO_SB', 'zone': '4'},
                'action': (('#vehicle-id', 'select[BMO_SB]'), ('#zone-id', 'select[4]')),
                'naics': ('all', 15, True),
                'vehicle': ('BMO_SB', 8, True),
                'pool': ('all', 18, 17, True),
                'zone': ('4', 7, True, True),
                'filters': (None, 0, True),
                'results': (5, 'results/csv/?vehicle=BMO_SB&zone=4'),
                'table': (5, 'h_naics_results', 'desc')
            },
            'bmo_zone_5': {
                'params': {'vehicle': 'BMO', 'zone': '5'},
                'action': (('#vehicle-id', 'select[BMO]'), ('#zone-id', 'select[5]')),
                'naics': ('all', 15, True),
                'vehicle': ('BMO', 8, True),
                'pool': ('all', 18, 17, True),
                'zone': ('5', 7, True, True),
                'filters': (None, 0, True),
                'results': (6, 'results/csv/?vehicle=BMO&zone=5'),
                'table': (6, 'h_naics_results', 'desc')
            },
            'bmo_sb_zone_6': {
                'params': {'vehicle': 'BMO_SB', 'zone': '6'},
                'action': (('#vehicle-id', 'select[BMO_SB]'), ('#zone-id', 'select[6]')),
                'naics': ('all', 15, True),
                'vehicle': ('BMO_SB', 8, True),
                'pool': ('all', 18, 17, True),
                'zone': ('6', 7, True, True),
                'filters': (None, 0, True),
                'results': (4, 'results/csv/?vehicle=BMO_SB&zone=6'),
                'table': (4, 'h_naics_results', 'desc')
            },
            
            'setasides_8a': {
                'params': {'setasides': 'A6'},
                'action': ('#eight_a', 'click'),
                'naics': ('all', 78, True),
                'vehicle': ('all', 5, True),
                'pool': ('all', 34, 59, True),
                'zone': ('all', 7, False, False),
                'filters': ('A6', 1, True),
                'results': (30, 'results/csv/?setasides=A6'),
                'table': (30, 'h_naics_results', 'desc'),
                'i1|xpath://*[@id="pool_vendors"]/tbody/tr[2]/td[5]/img': 'exists',
                'i2|xpath://*[@id="pool_vendors"]/tbody/tr[10]/td[5]/img': 'exists',
                'i3|xpath://*[@id="pool_vendors"]/tbody/tr[20]/td[5]/img': 'exists',
                'i4|xpath://*[@id="pool_vendors"]/tbody/tr[31]/td[5]/img': 'exists'
            },
            'setasides_hubz': {
                'params': {'setasides': 'XX'},
                'action': ('#Hubz', 'click'),
                'naics': ('all', 78, True),
                'vehicle': ('all', 5, True),
                'pool': ('all', 34, 59, True),
                'zone': ('all', 7, False, False),
                'filters': ('XX', 1, True),
                'results': (3, 'results/csv/?setasides=XX'),
                'table': (3, 'h_naics_results', 'desc'),
                'i1|xpath://*[@id="pool_vendors"]/tbody/tr[2]/td[6]/img': 'exists',
                'i2|xpath://*[@id="pool_vendors"]/tbody/tr[3]/td[6]/img': 'exists',
                'i3|xpath://*[@id="pool_vendors"]/tbody/tr[4]/td[6]/img': 'exists'
            },
            'setasides_sdvo': {
                'params': {'setasides': 'QF'},
                'action': ('#disvet', 'click'),
                'naics': ('all', 78, True),
                'vehicle': ('all', 5, True),
                'pool': ('all', 34, 59, True),
                'zone': ('all', 7, False, False),
                'filters': ('QF', 1, True),
                'results': (23, 'results/csv/?setasides=QF'),
                'table': (23, 'h_naics_results', 'desc'),
                'i1|xpath://*[@id="pool_vendors"]/tbody/tr[2]/td[7]/img': 'exists',
                'i2|xpath://*[@id="pool_vendors"]/tbody/tr[10]/td[7]/img': 'exists',
                'i3|xpath://*[@id="pool_vendors"]/tbody/tr[24]/td[7]/img': 'exists'
            },
            'setasides_wo': {
                'params': {'setasides': 'A2'},
                'action': ('#woman', 'click'),
                'naics': ('all', 78, True),
                'vehicle': ('all', 5, True),
                'pool': ('all', 34, 59, True),
                'zone': ('all', 7, False, False),
                'filters': ('A2', 1, True),
                'results': (16, 'results/csv/?setasides=A2'),
                'table': (16, 'h_naics_results', 'desc'),
                'i1|xpath://*[@id="pool_vendors"]/tbody/tr[2]/td[8]/img': 'exists',
                'i2|xpath://*[@id="pool_vendors"]/tbody/tr[10]/td[8]/img': 'exists',
                'i3|xpath://*[@id="pool_vendors"]/tbody/tr[17]/td[8]/img': 'exists'
            },
            'setasides_vo': {
                'params': {'setasides': 'A5'},
                'action': ('#vet', 'click'),
                'naics': ('all', 78, True),
                'vehicle': ('all', 5, True),
                'pool': ('all', 34, 59, True),
                'zone': ('all', 7, False, False),
                'filters': ('A5', 1, True),
                'results': (18, 'results/csv/?setasides=A5'),
                'table': (18, 'h_naics_results', 'desc'),
                'i1|xpath://*[@id="pool_vendors"]/tbody/tr[2]/td[9]/img': 'exists',
                'i2|xpath://*[@id="pool_vendors"]/tbody/tr[10]/td[9]/img': 'exists',
                'i3|xpath://*[@id="pool_vendors"]/tbody/tr[19]/td[9]/img': 'exists'
            },
            'setasides_sdb': {
                'params': {'setasides': '27'},
                'action': ('#smalldis', 'click'),
                'naics': ('all', 78, True),
                'vehicle': ('all', 5, True),
                'pool': ('all', 34, 59, True),
                'zone': ('all', 7, False, False),
                'filters': ('27', 1, True),
                'results': (30, 'results/csv/?setasides=27'),
                'table': (30, 'h_naics_results', 'desc'),
                'i1|xpath://*[@id="pool_vendors"]/tbody/tr[2]/td[10]/img': 'exists',
                'i2|xpath://*[@id="pool_vendors"]/tbody/tr[10]/td[10]/img': 'exists',
                'i3|xpath://*[@id="pool_vendors"]/tbody/tr[20]/td[10]/img': 'exists',
                'i4|xpath://*[@id="pool_vendors"]/tbody/tr[31]/td[10]/img': 'exists'
            }
        }
    }